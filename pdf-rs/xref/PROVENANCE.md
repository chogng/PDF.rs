...
`pdf-rs/xref` owns strict cross-reference primitives for the Native product core. A reusable
...
revision candidates. `pdf-rs/bytes` owns immutable source snapshots and byte delivery, while
`pdf-rs/syntax` owns direct-object syntax. The sibling `pdf-rs/object` crate validates one supplied
...
- The decoded entry is an untrusted semantic parser, not a filter-output attestation. It does not
  depend on `pdf-rs/filters`, inspect a filter plan, or mint a proof-shaped value. The owning document
...
`pdf-rs/xref::repository_policy` scans product source for forbidden filesystem, network,
async-runtime, and external-engine tokens and verifies that the crate depends only on
`pdf-rs/bytes` and `pdf-rs/syntax`. No registered coverage-guided fuzz target, pinned conformance
