...
fn product_raster_has_one_native_scene_dependency_and_no_platform_runtime() {
    ...
    for forbidden_table in ["[build-dependencies]", "[target."] {
        assert!(
            ...
            "pdf-rs/raster must not declare {forbidden_table} dependencies"
        );
    }
...
fn reference_foundation_keeps_atomic_bounded_scene_consumption_explicit() {
    ...
    for required in [
        ...
        "preflight_graphics(graphics, &mut graphics_preflight, &mut work)?",
...
fn m3_reference_pixel_foundation_is_traceable_without_maturity_overclaim() {
    ...
    let repository_root = crate_root
        ...
        .expect("pdf-rs/raster has a repository root two levels above it");
    let feature_map =
    ...
    for required in [
        ...
        "modules = [\"pdf-rs/font\", \"pdf-rs/document\", \"pdf-rs/content\", \"pdf-rs/scene\", \"pdf-rs/raster\"]",
