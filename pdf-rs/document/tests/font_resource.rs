...
fn foundational_cff() -> Vec<u8> {
    foundational_cff_with_encoding(None)
}

fn foundational_cff_with_encoding(encoding: Option<&[u8]>) -> Vec<u8> {
    ...
    let top_len = if encoding.is_some() { 18 } else { 12 };
    ...
    let encoding_offset = charstrings_offset + charstrings.len();
    let mut top = Vec::new();
    ...
    top.push(17);
    if encoding.is_some() {
        cff_dict_integer(&mut top, encoding_offset);
        top.push(16);
    }
...
#[test]
fn type1c_custom_encoding_does_not_override_the_pdf_font_dictionary() {
    // The CFF program would associate B and C with its two non-.notdef glyphs. The PDF Font
    // dictionary instead owns code selection through StandardEncoding and Differences.
    let program = foundational_cff_with_encoding(Some(&[0, 2, b'B', b'C']));
    let font = format!(
        "<< /Type /Font /Subtype /Type1 \
         /Encoding << /BaseEncoding /StandardEncoding /Differences [65 /A 97 /aacute] >> \
         /FirstChar 32 /LastChar 126 /Widths [{}] /FontDescriptor 5 0 R >>",
    ...
    let prepared = prepare(&fixture, 18_551);
    let ready = acquire_ready(&prepared, FontResourceLimits::default(), 18_561);

    assert_eq!(ready.glyph_id_for_code(b'A').unwrap().get(), 1);
    assert_eq!(ready.glyph_id_for_code(b'a').unwrap().get(), 2);
    assert_eq!(ready.glyph_id_for_code(b'B').unwrap().get(), 0);
    assert_eq!(ready.glyph_id_for_code(b'C').unwrap().get(), 0);
}
