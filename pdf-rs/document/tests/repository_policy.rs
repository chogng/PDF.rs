...
fn product_document_core_has_only_approved_sibling_dependencies_and_no_platform_io() {
    ...
    for forbidden_table in ["[dev-dependencies]", "[build-dependencies]", "[target."] {
        assert!(
            ...
            "pdf-rs/document must not declare {forbidden_table} dependencies"
        );
    }
...
fn m2_page_index_build_and_lazy_lookup_are_traceable_without_overclaim() {
    ...
    let repository_root = crate_root
        ...
        .expect("pdf-rs/document has a repository root two levels above it");
    let page_index =
...
fn m2_inherited_page_values_are_traceable_as_one_bounded_profile() {
    ...
    for required in [
        ...
        "modules = [\"pdf-rs/document\"]",
        "pdf-rs/document::page_materialization",
        "pdf-rs/document::repository_policy",
        "fuzz_targets = []",
        "benchmarks = []",
    ] {
        assert!(
    ...
    assert!(page_tree.contains("pdf-rs/document::page_materialization"));
...
fn m2_page_property_lookup_is_no_io_bounded_and_traceable() {
    ...
    for required in [
        ...
        "RPE-ARCH-001/15.3/M2",
        "modules = [\"pdf-rs/document\"]",
        "pdf-rs/document::page_properties",
        "pdf-rs/document::repository_policy",
        "fuzz_targets = []",
        "benchmarks = []",
    ] {
    ...
    assert!(document_model.contains("pdf-rs/document::page_properties"));
    ...
    assert!(milestone.contains("pdf-rs/document::page_properties"));
...
fn m2_page_content_acquisition_is_proof_bound_and_traceable() {
    ...
    assert!(stream_decode.contains("pdf-rs/document::page_content"));
    ...
    assert!(document_model.contains("pdf-rs/document::page_content"));
    assert!(document_model.contains("M2 adds four separate PLANNED document profiles"));
    assert!(document_model.contains("m2.page-property-lookup.v1"));
    assert!(
...
fn source_xref_stream_acquisition_stays_proof_bound_and_partial() {
    ...
    for required in [
        ...
        "modules = [\"pdf-rs/document\", \"pdf-rs/filters\", \"pdf-rs/object\", \"pdf-rs/xref\"]",
        "pdf-rs/document::source_xref_stream",
        "fuzz_targets = []",
        "benchmarks = []",
    ] {
        assert!(
...
fn source_revision_chain_acquisition_stays_proof_bound_and_partial() {
    ...
    for required in [
        ...
        "RPE-ARCH-001/5.6",
        "RPE-ARCH-001/15.3/M1",
        "modules = [\"pdf-rs/document\", \"pdf-rs/filters\", \"pdf-rs/xref\", \"pdf-rs/object\"]",
        "pdf-rs/document::source_revision_chain",
        "fuzz_targets = []",
        "benchmarks = []",
    ] {
        assert!(
...
fn source_acquired_document_services_stay_proof_bound_and_planned() {
    ...
    for required in [
        ...
        "RPE-ARCH-001/15.3/M1",
        "modules = [\"pdf-rs/document\"]",
        "pdf-rs/document::acquired_object",
        "pdf-rs/document::acquired_services",
        "pdf-rs/document::repository_policy",
        "fuzz_targets = []",
        "benchmarks = []",
    ] {
    ...
    for requirement_id in [
        ...
        assert!(
            requirement.contains("pdf-rs/document::acquired_object"),
...
fn traceability_registers_strict_base_open_as_a_planned_product_composition() {
    ...
    for required in [
        ...
        "RPE-STD-005/5-9",
        "modules = [\"pdf-rs/document\"]",
        "pdf-rs/document::strict_base_open",
        "pdf-rs/document::repository_policy",
...
fn traceability_registers_revision_resolution_without_claiming_complete_m1_support() {
    ...
    for required in [
        ...
        "RPE-STD-005/7-10",
        "modules = [\"pdf-rs/document\"]",
        "pdf-rs/document::revision_resolver",
        "pdf-rs/document::repository_policy",
        "fuzz_targets = []",
        "benchmarks = []",
    ] {
...
fn traceability_registers_core_repaired_open_without_claiming_a_session() {
    ...
    for required in [
        ...
        "pdf-rs/document::local_repair_geometry",
        "pdf-rs/document::local_repair_open",
...
fn traceability_registers_strict_page_count_without_claiming_a_page_index() {
    ...
    for required in [
        ...
        "RPE-ARCH-001/15.3/M1",
        "modules = [\"pdf-rs/document\"]",
        "pdf-rs/document::page_tree_count",
        "pdf-rs/document::page_tree_limit_config",
        "pdf-rs/document::repository_policy",
        "pdf-rs/document::local_repair_open",
        "pdf-rs/document::acquired_services",
        "tools/quality::native_object_loop",
...
fn traceability_registers_strict_outline_as_a_bounded_differential_profile() {
    ...
    for required in [
        ...
        "modules = [\"pdf-rs/document\"]",
        "pdf-rs/document::outline",
        "pdf-rs/document::outline_limit_config",
        "pdf-rs/document::repository_policy",
        "pdf-rs/document::local_repair_open",
        "pdf-rs/document::acquired_services",
        "tools/quality::m1_document_service_differential",
    ...
    for required in [
        ...
        "implementation = [\"pdf-rs/document\"]",
        "pdf-rs/document::outline",
        "pdf-rs/document::repository_policy",
        "status = \"partial\"",
