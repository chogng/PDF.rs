...
`pdf-rs/document` provides `OpenStrictBaseRevisionJob` as the resumable product composition for one
...
`pdf-rs/xref` owns traditional xref parsing, `pdf-rs/syntax` owns the supported header and direct-object
grammar, `pdf-rs/object` owns bounded indirect-object framing, and `pdf-rs/bytes` owns immutable
...
- `ResolveObjectJob` validates the selected uncompressed number, generation, offset, preceding
  whitespace, `obj` header, value, and `endobj` framing through `pdf-rs/object`. A primary xref-stream
...
- `RevisionObjectIndex::resolve_compressed` accepts only a complete `pdf-rs/object::ObjectStream`
...
- `AttestedRevisionIndex::count_pages` remains the borrowed factory for the one-shot page-count job,
  ...
  Catalog parser is shared within `pdf-rs/document`, but optional service fields remain lazily owned:
...
- `decode_text_string` consumes only an already lexical-decoded `PdfString`; literal/hexadecimal
  escaping remains owned by `pdf-rs/syntax`. A leading `FE FF` selects UTF-16BE and every other byte
