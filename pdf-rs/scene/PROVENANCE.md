...
`pdf-rs/scene` owns the immutable Native Scene v1 foundation and the incompatible graphics-capable
...
`pdf-rs/bytes` owns runtime source identity. `pdf-rs/syntax` owns `ObjectRef`. Document and content
