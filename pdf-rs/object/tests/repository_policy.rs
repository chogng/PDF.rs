...
fn product_object_core_only_depends_on_bytes_filters_and_syntax_and_has_no_platform_io() {
    ...
    assert_eq!(
        ...
        "pdf-rs/object may depend only on lower-level bytes, filters, and syntax crates"
    );
    assert!(
        !manifest.contains("pdf-rs-xref"),
        "pdf-rs/object must remain a sibling of pdf-rs/xref rather than depending on it"
    );
    assert!(
        !manifest.contains("[dev-dependencies]"),
        "pdf-rs/object must not introduce development dependencies"
    );
    ...
    assert!(
        !sources.is_empty(),
        "pdf-rs/object source selection is non-empty"
    );
    ...
    assert!(
        forbids_unsafe,
        "pdf-rs/object must forbid unsafe code at its crate boundary"
    );
}
...
fn traceability_registers_staged_stream_length_without_claiming_a_resolver() {
    ...
    let repository_root = crate_root
        ...
        .expect("pdf-rs/object has a repository root two levels above it");
    let feature_map =
    ...
    for required in [
        ...
        "modules = [\"pdf-rs/xref\", \"pdf-rs/object\"]",
        "pdf-rs/xref::final_startxref",
        "pdf-rs/xref::xref_anchor",
        "pdf-rs/object::object_behavior",
        "pdf-rs/object::repository_policy",
        "fuzz_targets = []",
        "benchmarks = []",
    ] {
        assert!(
    ...
    for required in [
        ...
        "modules = [\"pdf-rs/object\"]",
        "pdf-rs/object::staged_length",
        "pdf-rs/object::object_behavior",
        "pdf-rs/object::repository_policy",
        "fuzz_targets = []",
        "benchmarks = []",
    ...
    for requirement in [syntax_requirement, object_requirement, milestone] {
        ...
        assert!(requirement.contains("pdf-rs/object::staged_length"));
