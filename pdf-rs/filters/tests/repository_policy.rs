...
fn product_filters_only_depend_on_bytes_and_syntax_and_have_no_platform_io() {
    ...
    assert_eq!(
        ...
        "pdf-rs/filters may depend only on lower-level bytes and syntax primitives"
    );
    assert!(
        !manifest.contains("[dev-dependencies]"),
        "pdf-rs/filters must not introduce development dependencies"
    );
...
fn traceability_registers_basic_filters_without_claiming_stream_integration() {
    ...
    let repository_root = crate_root
        ...
        .expect("pdf-rs/filters has a repository root two levels above it");
    let feature_map =
    ...
    for required in [
        ...
        "RPE-ARCH-001/5.6",
        "modules = [\"pdf-rs/filters\"]",
        "pdf-rs/filters::decode_behavior",
        "pdf-rs/filters::predictor_behavior",
        "pdf-rs/filters::repository_policy",
        "fuzz_targets = []",
        "benchmarks = []",
    ] {
        assert!(
