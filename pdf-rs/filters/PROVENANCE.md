...
`pdf-rs/filters` is the first bounded Native product stream-decoding slice. It consumes one exact,
...
cancellation probes, and source-redacted errors. `pdf-rs/bytes` owns immutable snapshot-backed
physical storage. `pdf-rs/syntax` owns parsed direct dictionaries, checked physical spans, and object
...
`../pdfium/pdf-rs/fxcodec/flate/flatemodule.cpp` predictor dispatch while checking the reported PNG
...
`pdf-rs/filters::repository_policy` enforces the two lower-level product dependencies, absence of
...
- Empty encoded streams cannot currently be represented by the non-empty `ByteRange`/`ByteSlice`
  primitive. Resolving that physical-input contract belongs to `pdf-rs/bytes`; this crate does not
