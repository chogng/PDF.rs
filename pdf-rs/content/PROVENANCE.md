...
`pdf-rs/content` owns the bounded M2-05 scanner, the sealed M2-06 initial Content VM, and the
...
`pdf-rs/font` owns bounded TrueType table parsing, printable WinAnsi mapping, metrics, and exact
...
`pdf-rs/content` owns decoded lexical scanning, operator classification, known-operator operand
