...
fn content_crate_has_only_the_approved_layered_dependencies() {
    ...
    for forbidden_table in ["[build-dependencies]", "[target."] {
        assert!(
            ...
            "pdf-rs/content must not declare {forbidden_table} dependencies"
        );
    }
    for forbidden in [
        "pdfium",
...
fn bounded_content_profiles_remain_planned_after_m2_and_m3_work_items_close() {
    ...
    let repository_root = crate_root
        ...
        .expect("pdf-rs/content has a repository root two levels above it");
    ...
    for required in [
        ...
        "modules = [\"pdf-rs/content\", \"pdf-rs/scene\"]",
