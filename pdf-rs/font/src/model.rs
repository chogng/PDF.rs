...
impl CffFont {
    ...
    /// Resolves one byte through PDF WinAnsiEncoding.
    pub fn glyph_id_for_winansi_code(&self, byte: u8) -> Option<GlyphId> {
        self.glyph_id_for_name(winansi_encoding_name(byte)?)
    }
    /// Returns the unscaled horizontal advance for one valid glyph identifier.
    pub fn advance_width(&self, glyph_id: GlyphId) -> Option<u16> {
        self.glyphs
...
fn winansi_encoding_name(byte: u8) -> Option<&'static str> {
    const NAMES: [&str; 224] = [
        "space",
        "exclam",
        "quotedbl",
    ...
    NAMES.get(usize::from(byte.checked_sub(0x20)?)).copied()
}
