...
constexpr unsigned long kMaxTitleUtf16Bytes = 4096;
constexpr size_t kMaxTotalTitleUtf16Bytes = 64U * 1024U;
constexpr size_t kMaxJsonBytes = 1024U * 1024U;
...
std::optional<uint16_t> ReadU16(std::span<const uint8_t> bytes,
                                size_t offset) {
...
bool WriteResponse(const Request& request,
                   std::optional<std::string> parse_json) {
  const bool success = parse_json.has_value();
...
bool AppendUtf8(uint32_t scalar, std::string* output) {
  if (scalar <= 0x7fU) {
...
std::optional<std::string> ReadTitle(FPDF_BOOKMARK bookmark,
                                     size_t* total_title_bytes) {
  const unsigned long required = FPDFBookmark_GetTitle(bookmark, nullptr, 0U);
  if (required < 2U || required > kMaxTitleUtf16Bytes || required % 2U != 0U ||
      *total_title_bytes > kMaxTotalTitleUtf16Bytes - required) {
    return std::nullopt;
  }
  std::vector<uint8_t> bytes(required);
  if (FPDFBookmark_GetTitle(bookmark, bytes.data(), required) != required ||
      bytes[required - 2U] != 0U || bytes[required - 1U] != 0U) {
    return std::nullopt;
  }
  *total_title_bytes += required;

  std::string output;
  output.reserve(required);
  const size_t units = required / 2U - 1U;
  size_t index = 0;
  while (index < units) {
    const uint16_t first =
        static_cast<uint16_t>(static_cast<uint16_t>(bytes[index * 2U]) |
                              (static_cast<uint16_t>(bytes[index * 2U + 1U])
                               << 8U));
    ++index;
...
std::optional<std::string> ObserveOutline(FPDF_DOCUMENT document) {
  std::string output = "{\"schema\":1,\"items\":[";
  std::vector<PendingItem> work;
  work.reserve(kMaxDepth * 2U);
  std::unordered_set<FPDF_BOOKMARK> visited;
  visited.reserve(kMaxItems);
  if (FPDF_BOOKMARK root = FPDFBookmark_GetFirstChild(document, nullptr)) {
    work.push_back({root, 1U});
  }
  size_t total_title_bytes = 0;
  size_t item_count = 0;

  while (!work.empty()) {
    const PendingItem pending = work.back();
    work.pop_back();
    if (!pending.bookmark || pending.depth == 0U ||
        pending.depth > kMaxDepth || item_count == kMaxItems ||
        !visited.insert(pending.bookmark).second) {
      return std::nullopt;
    }
    const auto title = ReadTitle(pending.bookmark, &total_title_bytes);
    if (!title) {
      return std::nullopt;
    }
    if (item_count != 0U) {
    ...
    output.append("{\"depth\":");
    output.append(std::to_string(pending.depth));
    output.append(",\"title\":");
    AppendJsonString(*title, &output);
    output.append(",\"count\":");
    output.append(std::to_string(FPDFBookmark_GetCount(pending.bookmark)));
    output.append(",\"target\":\"");
    if (FPDFBookmark_GetAction(pending.bookmark)) {
      output.append("action");
    } else if (FPDFBookmark_GetDest(document, pending.bookmark)) {
    ...
    const FPDF_BOOKMARK sibling =
        FPDFBookmark_GetNextSibling(document, pending.bookmark);
    const FPDF_BOOKMARK child =
        FPDFBookmark_GetFirstChild(document, pending.bookmark);
    if (sibling) {
      work.push_back({sibling, pending.depth});
    }
    if (child) {
      if (pending.depth == kMaxDepth) {
        return std::nullopt;
      }
      work.push_back({child, pending.depth + 1U});
    }
  }
  output.append("]}\n");
  if (output.size() > kMaxJsonBytes) {
...
bool Run() {
  ...
  ScopedDocument document(FPDF_LoadMemDocument64(
      request->pdf.data(), request->pdf.size(), nullptr));
  if (!document.get()) {
    return WriteResponse(*request, std::nullopt);
  }
  return WriteResponse(*request, ObserveOutline(document.get()));
}
