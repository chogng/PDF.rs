...
traditional xref section and candidate physical index into a fully attested
strict base-revision index, then back through its proof-preserving object-access
capability into all four parsed values, through its strict Catalog/page-tree
capability to a scalar page count, and through its bounded top-level
...
It runs the product xref job, builds a `CandidateRevisionIndex`, then consumes
that untrusted candidate through `AttestRevisionJob`. The job linearly validates
...
candidate physical interval, strict header-to-`startxref` trivia coverage, every
