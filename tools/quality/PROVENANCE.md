...
capability into all four parsed values.
...
- Passing the Native object loop proves one generated traditional-table fixture
  ...
  boundary. Access caps are per job and do not provide resolver-wide aggregate
  work accounting. This test does not provide a revision/reference resolver,
  cycle states, cache, xref-stream, hybrid, `/Prev` or object-stream support,
  repair, R0 or ISO conformance, filter decoding, content interpretation,
  rendering, or Native/PDFium semantic or pixel differential required by later
  milestones.
