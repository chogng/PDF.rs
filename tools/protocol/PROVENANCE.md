...
- `fixed_le_v1` codecs are generated for both Rust and TypeScript. CapabilityDecision and
  ...
  fixed_le_v1 payload`; generated known answers freeze both framing and digest.
- `generate <repository-root>` is the only writing mode. `--check <repository-root>` regenerates in
  ...
  mode stages the complete output set under one repository lock and rolls back a partial replace.
...
strict CLI arity with usage exit 2, oversized-schema preflight, and a real temporary-checkout proof
that check mode preserves generated bytes and modification times. Browser runtime tests execute
generated validators and typed codecs for message/header/correlation/transfer binding and Surface
stride/range invariants.
