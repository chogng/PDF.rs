...
#[test]
fn product_raster_has_only_native_policy_and_scene_dependencies_and_no_platform_runtime() {
    ...
    let policy_manifest = fs::read_to_string(crate_root.join("../../runtime/policy/Cargo.toml"))
        .expect("policy manifest must be readable");
    assert!(
        !policy_manifest.contains("pdf-rs-raster"),
        "policy must not depend upward on its Fast raster consumer"
    );
