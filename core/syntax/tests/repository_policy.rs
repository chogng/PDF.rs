...
fn product_syntax_core_only_depends_on_bytes_and_has_no_platform_io() {
    ...
    assert_eq!(
        ...
        "core/syntax may depend only on the lower-level core/bytes crate"
    );
    assert!(
        !manifest.contains("[dev-dependencies]"),
        "core/syntax must not introduce development dependencies"
    );
    ...
    assert!(
        !sources.is_empty(),
        "core/syntax source selection is non-empty"
    );
