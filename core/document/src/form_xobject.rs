...
pub enum FormXObjectUnsupportedKind {
    ...
    NonFormXObject,
    /// `/Resources` is absent, indirect, or otherwise outside the direct-dictionary subset.
    UnsupportedResources,
    /// The Form declares a filter outside the initial identity payload subset.
    UnsupportedFilter,
...
/// Runtime identity and exact checkpoints for one Form object and payload acquisition.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
pub enum FormXObjectPhase {
    ...
    Object,
    /// Reading the exact identity-encoded Form content payload.
    Payload,
...
/// One proof-bearing identity-encoded Form XObject and its direct resource scope.
...
impl AcquiredFormXObject {
    ...
    /// Borrows the exact identity-encoded Form content bytes.
    pub fn content_bytes(&self) -> &[u8] {
        self.content.bytes()
    }

    /// Borrows the exact source-bound content slice.
    pub const fn content(&self) -> &ByteSlice {
        &self.content
    }
...
/// Resumable proof-bound acquisition of one identity-encoded Form XObject.
...
impl AcquireFormXObjectJob {
    ...
    pub fn poll(
        ...
        match &self.state {
            ...
            FormState::Object | FormState::Payload => {}
        }
            ...
            match &mut self.state {
                FormState::Object => {
                    ...
                    match outcome {
                        ...
                        AttestedObjectPoll::Ready(object) => {
                            match inspect_form(&object, &mut self.stats) {
                                Ok(Ok(metadata)) => {
                ...
                FormState::Payload => {
                    ...
                    let retained = match object
                        .syntax_heap_bytes()
                        .checked_add(u64::try_from(bytes.bytes().len()).unwrap_or(u64::MAX))
                    {
                        Some(value) if value <= MAX_RETAINED_BYTES => value,
                        _ => return self.fail(invalid_form(self.proof.target(), span.start())),
                    };
                    let resources = match PageResourceScope::form(object, metadata.resources_offset)
                    {
...
impl SharedAttestedRevisionIndex {
    ...
    pub fn acquire_form_xobject(
        ...
        if context.object_envelope_checkpoint() == context.object_boundary_checkpoint()
            || context.object_envelope_checkpoint() == context.payload_checkpoint()
            || context.object_boundary_checkpoint() == context.payload_checkpoint()
        {
            return Err(DocumentError::for_code(
...
fn inspect_form(
    ...
    if entries
        .filter
        .is_some_and(|value| !matches!(value.value(), SyntaxObject::Null))
        || entries
            .decode_parameters
            .is_some_and(|value| !matches!(value.value(), SyntaxObject::Null))
    {
    ...
    let resources_offset = match entries.resources {
        Some(value) if matches!(value.value(), SyntaxObject::Dictionary(_)) => value.span().start(),
        Some(value) => {
            return Ok(Err(FormXObjectUnsupported::new(
...
fn parse_rectangle(value: &SyntaxObject) -> Option<PageRectangle> {
    ...
    PageRectangle::new([
        parse_coordinate(left.value())?,
        parse_coordinate(bottom.value())?,
        parse_coordinate(right.value())?,
        parse_coordinate(top.value())?,
    ])
}
