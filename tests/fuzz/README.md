...
No fuzz target is registered in M0. `fuzz_targets` therefore remains empty in the feature map.
...
This smoke test has no coverage guidance, random loop, time budget, corpus growth, sanitizer,
...
input. Registered targets, coverage gates, watchdog-backed campaigns, and minimization remain open.
