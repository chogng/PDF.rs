...
subject、profile/policy version、scope、missing requirements 和 contributors：
...
`GenerationPlanned` 携带 `RenderPlanManifest` 与 `plan_hash`。Manifest 绑定 document revision、
render config、renderer epoch、plan ID、Scene hash、decision hash、Native backend、output
profile、quality 和唯一非空 regions。`plan_hash` 对 exact manifest 使用独立 domain：
