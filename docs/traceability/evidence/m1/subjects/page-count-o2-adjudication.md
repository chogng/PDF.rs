# Page-count O2 adjudication

The content-addressed input `document/m1-adjudication/ill-typed-optional-references` places integer `42` in the optional root `Pages.Parent` slot. The frozen pre-fix projection treated that present-but-ill-typed value as absent and returned page count 1. Native instead returns `RPE-DOCUMENT-0032`, and the independent reference returns its semantic-rejection outcome on the identical bytes.
