...
- 超出 R0 矩阵的复杂 encoding/CMap/CID 映射、RTL、竖排 CIDFont、ActualText、Tagged PDF、结构化可访问性、面向新写入文本的 shaping，以及受控字体 fallback 的决策与条件实施，属于独立的 Post-R0 Font/Text 里程碑 FT1；其可执行计划为 [`plan/post-r0-font-text.toml`](../../plan/post-r0-font-text.toml)。
...
R0 对缺失或非嵌入字体继续 fail-closed，并返回结构化 `UnsupportedCapability`。Post-R0 若研究受控 fallback，必须先通过独立决策 gate；获批的固定字体包和系统字体 adapter 使用不同 CapabilityProfile，绑定字体文件哈希或完整平台字体环境指纹、选择规则、metrics 兼容策略、平台矩阵和缓存 epoch。仅枚举当前机器的系统字体不构成可复现能力，也不得改变 R0 的字形或排版结果。
...
#### 7.2.1 Post-R0 复杂 encoding 与 CID 映射

M6 只关闭 `release-r0-v1` 注册的常见水平矩阵。FT1 必须先冻结更广的、按 family 独立计数的矩阵，再实现以下能力：

- simple font 的 BaseEncoding、Differences、symbolic 映射与 glyph-name 数据来源；
- embedded/predefined CMap、`usecmap` 图、1–4 byte codespace、Registry/Ordering/Supplement 与版本兼容；
- CIDFontType0/CFF、CIDFontType2、`CIDToGIDMap` Identity/stream、缺失 CID/glyph 策略；
- collection Unicode map 与 ToUnicode 的优先级、冲突、multi-scalar、non-BMP 和 `Unknown` 路径；
- Identity-V/预定义竖排 CMap、`W2`/`DW2`、vertical origin 与 glyph orientation。

encoding、glyph-name、CMap 和 CID collection 数据必须通过已批准来源的生成器进入仓库，并绑定许可证、输入/输出哈希和更新策略。每个 collection/encoding family 独立提供 O0/O1、canonical mapping diff、fuzz/minimization、holdout 和 reviewer 决策；不能用一个总 CJK 百分比掩盖失败类别。

#### 7.2.2 Shaping 的生产边界

Shaping 是 Post-R0 的独立 authoring 能力，不是 reader 的字体 fallback 技巧。其输入是 Unicode、script、language、direction、OpenType features、variation coordinates 和已选择字体；输出是 glyph/cluster、advance/offset 及 provenance。随后必须由 writer 生成确定性的 subset/embed、Widths/vertical metrics、encoding/CMap、ToUnicode、资源和 content operators，并由正常 reader 重新打开验证。

具体 shaper 可由项目自有实现或经批准的叶子库提供，但依赖 ADR 必须记录 semantic ownership、license/SBOM、source/checksum、Wasm/native、budget/cancellation adapter、unsafe/build script、替换计划和 conformance 版本。编译期 dependency-direction 与运行时 trace 必须证明现有 PDF 的 parser、Content VM、Scene replay、extract/search/copy 调用图无法到达 shaper。

#### 7.2.3 受控字体 fallback

字体 fallback 决策允许四种明确结果：继续 fail-closed、只批准固定字体包、只批准有界系统字体 adapter、或把后二者作为两个独立 profile 批准。任何批准只激活实施工作，不自动改变默认策略。

系统字体 adapter 只能返回有界候选 manifest：canonical family/style、face index、不可变 bytes/approved handle、文件 SHA-256、完整字体环境 fingerprint、来源/许可证决策和 platform epoch。Core 拥有确定性选择、glyph coverage、PDF widths/vertical metrics/bounds 阈值和拒绝策略；并列、环境变化、文件 TOCTOU、阈值失败或权限不足均返回结构化 Unsupported。

fallback 仅适用于决策中列明的 missing/non-embedded font 类别，不能掩盖 malformed embedded font、unsupported mapping 或内部错误。它保留原 PDF 的 character boundaries、glyph positions、advances、`Tj`/`TJ` 和 text matrices，不得调用 authoring shaper 重排已定位文字。其 provenance/confidence 必须进入 Text、Pixel、diagnostic、font-environment hash、renderer epoch 和 cache key，并提供 kill switch 与回滚演练。
...
| FT1 Post-R0 advanced Font/Text | 超出 R0 的复杂 encoding/CMap/CID 映射、RTL、竖排 CIDFont、ActualText、Tagged PDF、structure/a11y、authoring-only shaping/writer round trip，以及受控字体 fallback 的决策与条件实施 | 各能力由互不混淆的 profile 和 O0-O3/fuzz/holdout/E2E/round-trip 证据晋级；fallback 未获 ADR 批准时保持 fail-closed；不得追溯扩大 R0 |
...
里程碑是依赖 gate，不是日历承诺。项目启动前必须增加 `plan/r0.toml`，为 M0-M6 填写 owner、reviewer、计划 FTE、开始/目标日期、依赖、容量假设和最大并行工作数；未填写前本文档保持“候选稿”状态。关键依赖链为 M0 → M1 → M2 → M3 → M4 → M6；M5 可在 protocol/surface schema 冻结后与 M3/M4 后半段并行，但不能绕过 M4 的 CapabilityDecision 和 Native RenderPlan 契约。M6 的细化交付物、依赖与退出条件由 `plan/m6.toml` 管理；FT1 由 `plan/post-r0-font-text.toml` 管理，只有 M6/R0 退出后才能开始能力晋级。FT1 的 authoring writer integration 还必须等待 M8 ChangeSet/incremental-writer 契约冻结；fallback 的实现工作仅在其决策 gate 批准某一精确 profile 后激活。
