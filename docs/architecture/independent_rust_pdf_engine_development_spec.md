...
PDF 字体系统必须把“画出字形”和“理解文字”分成两条链。PDF 通常已经给出字形位置，因此不能让通用排版引擎重新 shaping；系统字体只用于缺失字体的受控 fallback。
...
| 非嵌入字体 | 标准 14 字体映射 + 平台 font database adapter | 跨平台替代差异、宽度一致性、可诊断性 |
...
| 路径 | 数据流 | 使用条件 |
| --- | --- | --- |
| Worker + OffscreenCanvas | 主线程 transfer canvas → Worker 直接绘制 | 首选；标准允许将 canvas 控制权转移给 worker [R5] |
| Worker CPU + ImageBitmap | Wasm raster → bitmap → transferable ImageBitmap | OffscreenCanvas 或 WebGPU 不可用时 |
...
- 屏幕阅读器树不应简单复用视觉 span；应优先使用 structure tree、ActualText、Alt 和逻辑顺序。
...
| --- | --- | --- |
| 控制消息 | 结构化小消息 | 相同 |
| 大型像素 | OffscreenCanvas / ImageBitmap | transferable ArrayBuffer |
...
`BrowserTransfer` 的 slot 必须对应同一 `postMessage` transfer list 中的实际 `ImageBitmap` 或 `ArrayBuffer`，不是跨 realm 全局 token。普通 Wasm Memory 的 `ptr` 只能由同 Worker 的 JS glue 读取；若需要发往主线程，必须复制/转移到 `ArrayBuffer`、创建 `ImageBitmap`，或在满足跨源隔离时使用经协商的 SharedArrayBuffer。Wasm memory grow 会增加 `memory_epoch`；旧 local surface 在 epoch 不匹配时拒绝访问。
...
| M4 Fast CPU + 桌面 vertical slice | tile、调度、完整缓存键、共享内存、Native RenderPlan、CapabilityDecision | R0 eligible 基础页面进入 CANARY；桌面打开/滚动/缩放闭环可用 |
| M5 浏览器 vertical slice | 单一 Native Engine Worker、OffscreenCanvas、transferable surface、TS viewer | 三浏览器引擎完成 R0 阅读流程；资源/依赖扫描证明无外部 PDF 引擎 |
| M6 R0 Release Candidate | Unicode、选择、复制、搜索、链接、release-r0-v1、固定硬件 perf | 第 2.6 节全部 R0 gate 通过；本里程碑对应第 16.3 节“首个可用版本” |
...
里程碑是依赖 gate，不是日历承诺。项目启动前必须增加 `plan/r0.toml`，为 M0-M6 填写 owner、reviewer、计划 FTE、开始/目标日期、依赖、容量假设和最大并行工作数；未填写前本文档保持“候选稿”状态。关键依赖链为 M0 → M1 → M2 → M3 → M4 → M6；M5 可在 protocol/surface schema 冻结后与 M3/M4 后半段并行，但不能绕过 M4 的 CapabilityDecision 和 Native RenderPlan 契约。
